...
//        if !self.nameOnCardTextField.hasText() {
//            showToastWithMessage(WalletStrings.enterName)
//            return false
//        }
        if self.firstCardNumberTextField.text == "" {
            showToastWithMessage(WalletStrings.enterCardNo)
            return false
        }

        if self.firstCardNumberTextField.text!.characters.count < 16 {
            showToastWithMessage(WalletStrings.validCardNo)
            return false
        }
        if !self.expiryMonthTextField.hasText {
            showToastWithMessage(WalletStrings.enterMonth)
            return false
        }
        if !self.expiryYearTextField.hasText {
            showToastWithMessage(WalletStrings.enterYear)
            return false
        }
        if !self.cvvNumberTextField.hasText {
            showToastWithMessage(WalletStrings.enterCvv)
            return false
        }
        if self.cvvNumberTextField.text?.characters.count < 3 {
            showToastWithMessage(WalletStrings.validCvv)
            return false
        }
...
//                    if let msg = error.userInfo["NSLocalizedDescription"] as? String{
//                        
...
//                    }
                    ...
                    if self.isCardAdd{
                        ...
                        ServiceClass.addDriverCardAPI(params, completionHandler: { (data) in
                            
                            if let stripe = data["stripe"].string{
...
//                        addDriverCardAPI(params, SuccessBlock: { (result: JSONDictionary) in
//                            
//                            printlnDebug(result)
//                            
//                            if let stripe = result["stripe"] as? String{
//                                
//                                UserDefaults.save(stripe as AnyObject, forKey: NSUserDefaultKey.STRIPE_ID)
//                                
//                            }
...
//                        })
                        ...
                        var params = JSONDictionary()
                        params["token"] = token
                        params["amount"] = self.amt as AnyObject
                        printlnDebug(params)
    ...
    func initialSetup() {
        ...
        self.navigationTitleLabel.text = TRANSACTION_DETAILS.localized
        self.cardHolderNameLabel.text = CARD_HOLDER_NAME.localized
        self.expiryLabel.text = EXPIRY.localized
        self.monthLabel.text = MONTH.localized
        self.yearLabel.text = YEAR.localized
        self.cvvLabel.text = CVV.localized
        self.cardNumberLabel.text = CARD_NUMBER.localized
        self.payNowBtn.setTitle(C_PAY_NOW.localized, for: .normal)
